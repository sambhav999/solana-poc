...
export default function App() {
  ...
  const refresh = useCallback(async () => {
    if (!connection) { setRules([]); setReceipts([]); return; }
    setLoading(true);
    try {
      const [r, rec] = await Promise.all([
        api.listRules(connection.address),
        api.receipts(connection.address).catch(() => ({ receipts: [] })),
      ]);
      setRules(r.rules || []);
    ...
    <div className="wrap">
      ...
      {tab === 'rules' && (
        <>
          ...
          {connection && loading && <div className="card empty"><span className="spinner" /> Reading positions and corporate actions…</div>}

          {connection && !loading && !rules.length && !creating && (
            <div className="card empty">
          ...
          {connection && rules.some((r) => r.sourceType === 'KAMINO_USDC') && (
